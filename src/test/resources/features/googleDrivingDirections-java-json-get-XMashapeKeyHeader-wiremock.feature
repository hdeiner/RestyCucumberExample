Feature: Google Maps Driving Directions
  ...
  Scenario Outline: Home to N07
    Given I am at "28 Lincoln Park Road, Pequannock NJ"
    When I want to go to "Lincoln Park Airport, Lincoln Park NJ"
    Then I should see that the total Distance is "1.5 mi", which should take "3 mins"
  ...
  Scenario Outline: Atrium to National Car Rental return at KORD
    Given I am at "3800 Gulf Road, Rolling Meadows IL"
    And I want to go to "560 Bessie Coleman Dr, Chicago"
    Then I should see that the total Distance is "13.0 mi", which should take "18 mins"
    And leg "<leg>", "<instruction>", for "<timeToTravel>", which should take "<distanceToTravel>" is returned
    Examples:
      |leg|instruction                                                                                              |timeToTravel|distanceToTravel|
      ...
      |12 |Turn left                                                                                                |1 min       |0.1 mi          |
      |13 |Turn right Destination will be on the right                                                              |1 min       |39 ft           |